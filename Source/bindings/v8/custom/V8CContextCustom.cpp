...
{
    if (args.Length() != 1)
        return throwError(v8SyntaxError, "CContext cannot be constructed because of wrong parameters.", args.GetIsolate());
    if (!V8CPlatform::HasInstance(args[0], args.GetIsolate(), worldType(args.GetIsolate())))
        return throwTypeError("CContext cannot be constructed because of wrong parameters.", args.GetIsolate());
...
v8::Handle<v8::Value> V8CContext::compileKernelMethodCustom(const v8::Arguments& args)
{
    ...
    if (!cKernel)
        return throwError(v8SyntaxError, "Cannot create new CKernel object.", args.GetIsolate());
...
v8::Handle<v8::Value> V8CContext::mapDataMethodCustom(const v8::Arguments& args)
{
    ...
    if (V8Int8Array::HasInstance(MAYBE_MISSING_PARAMETER(args, 0, DefaultIsUndefined), args.GetIsolate(), worldType(args.GetIsolate()))) {
        ...
        if (!cData)
            return throwError(v8SyntaxError, "Cannot create new CData object.", args.GetIsolate());
...
v8::Handle<v8::Value> V8CContext::getAlignmentOffsetMethodCustom(const v8::Arguments& args)
{
    ...
    return throwError(v8SyntaxError, "Cannot get alignment offset.", args.GetIsolate());
}
